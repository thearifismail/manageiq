class TransformationMappingItem < ApplicationRecord
  ...
  def source_network
    ...
    src_cluster_lans = tmin.collect(&:source).collect(&:lans).flatten
    source_lan       = source
  ...
  def destination_network
    ...
    if destination.kind_of?(Lan) # red hat
      ...
      dst_cluster_lans = tmin.collect(&:destination).collect(&:lans).flatten
      ...
      dst_cluster_lans = tmin.collect(&:destination).collect(&:cloud_networks).flatten
    end
